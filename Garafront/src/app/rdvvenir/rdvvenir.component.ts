...
@Component({
  selector: 'app-rdvvenir',
  templateUrl: './rdvvenir.component.html',
  styleUrls: ['./rdvvenir.component.css']
})
export class RdvvenirComponent implements OnInit {
  rdvs: any[] = [];
  filteredRdvs: any[] = [];
  ...
  hasReceivedQuotes(): boolean {
    return this.rdvs.some(rdv => rdv.devisrec);
  }
  ngOnInit(): void {
    this.fetchrdv();
    this.fetchVehicles();
  ...
  filterUpcomingAppointments(): void {
    const today = moment();
    this.filteredRdvs = this.rdvs.filter(rdv => moment(rdv.datesouhaite).isAfter(today));
  }
